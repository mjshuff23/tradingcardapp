...
export class CatalogController {
  ...
  @ApiOperation({
    summary: "Normalize a messy card title into structured fields",
  })
  ...
  @ApiOperation({
    summary:
      "Get the controlled card taxonomy used for category and subcategory inputs",
  })
  ...
  @Get()
  @ApiOperation({
    summary: "List cards with optional search and status filters",
  })
  ...
  @ApiOperation({
    summary: "Get a specific card image kind (front/back/canonical)",
  })
  @ApiBadRequestResponse({
    description: "kind must be front, back, or canonical.",
  })
  @ApiNotFoundResponse({ description: "Card or image not found." })
  async getCardImageByKind(
    ...
    const image = await this.catalogService.getCardImageByKind(
      cardId,
      viewer.id,
      imageKind,
    );
  ...
  @ApiOperation({
    summary: "Upload or replace a card image (front/back/canonical)",
  })
  ...
  async uploadCardImage(
    ...
    const imageKind = toImageKind(kind);
    return this.catalogService.uploadCardImage(
      cardId,
      user.id,
      imageKind,
      file,
