...
export function parseStructuredCardHints(
  frontText: string,
  backText?: string,
): StructuredCardHints {
  ...
  const seasons = unique(
    Array.from(
      lowered.matchAll(
        /\b(19\d{2}|20\d{2})\s*[-/]\s*(\d{2}|19\d{2}|20\d{2})\b/g,
      ),
    ).map((match) => {
  ...
  const years = unique(
    [
      ...Array.from(normalized.matchAll(/\b(19\d{2}|20\d{2})\b/g)).map(
  ...
  const subsets = SUBSET_KEYWORDS.filter((subset) =>
