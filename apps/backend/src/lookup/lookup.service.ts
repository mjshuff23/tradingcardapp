...
export class LookupService {
  ...
  async lookup(input: LookupInput): Promise<LookupResult> {
    ...
    for (const provider of providers) {
      try {
        ...
        this.logger.warn(`Lookup provider ${provider} failed: ${(error as Error).message}`);
      }
    }
  ...
  private async lookupWithGoogleVision(input: LookupInput): Promise<LookupResult> {
    ...
    const targets = [input.frontBuffer, input.backBuffer].filter(Boolean) as Buffer[];
    ...
    for (const imageBuffer of targets) {
      ...
      const pages = (data.pagesWithMatchingImages ?? []).filter((page) => !!page.url).slice(0, 6);
  ...
  private async lookupWithDuckDuckGo(input: LookupInput): Promise<LookupResult> {
