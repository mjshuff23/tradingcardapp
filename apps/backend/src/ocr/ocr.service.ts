...
import {
  parseStructuredCardHints,
  StructuredCardHints,
} from "../common/card-hints.util";
...
export class OcrService implements OnModuleDestroy, OnModuleInit {
  ...
  async extractText(input: OcrExtractInput): Promise<OcrExtractResult> {
    ...
    try {
      const [frontText, backText] = await Promise.all([
        ...
        input.backBuffer
          ? this.extractTextFromSingleImage(input.backBuffer, "back")
          : Promise.resolve(""),
      ]);

      const combined = [frontText, backText]
        .filter(Boolean)
        .join(" ")
        .replace(/\s+/g, " ")
        .trim();
  ...
  async onModuleDestroy() {
    ...
    try {
      ...
      this.logger.warn(
        `Failed to terminate OCR worker cleanly. ${(error as Error).message}`,
      );
    } finally {
  ...
  private fallbackResult(
    ...
    const backGuess =
      backFilename && backBuffer
        ? this.fallbackSingle(backFilename, backBuffer)
        : "";
  ...
  private fallbackSingle(sourceFilename: string, fileBuffer: Buffer): string {
    ...
    const sizeHint =
      fileBuffer.length > 0
        ? ` image-bytes-${Math.min(fileBuffer.length, 999999)}`
        : "";
    return `${guessFromFilename}${sizeHint}`.trim();
  }

  private async extractTextFromSingleImage(
    fileBuffer: Buffer,
    ...
    for (const variant of variants) {
      for (const mode of fullPassModes) {
        const attempt = await this.runAttempt(
          worker,
          variant.buffer,
          `${sideLabel}:${variant.name}:${mode.name}`,
          mode.psm,
        );
        if (attempt) {
    ...
    for (const region of regionBuffers) {
      const mode =
        region.name === "top" || region.name === "bottom"
          ? Tesseract.PSM.SINGLE_LINE
          : Tesseract.PSM.AUTO;
      const attempt = await this.runAttempt(
        worker,
        region.buffer,
        `${sideLabel}:region:${region.name}`,
        mode,
      );
      if (attempt) {
        attempts.push(attempt);
  ...
  private async createRegionBuffers(imageBuffer: Buffer): Promise<
    Array<{
      name: "top" | "bottom" | "right" | "center" | "left" | "numberStrip";
      buffer: Buffer;
    }>
  > {
    ...
    const [top, bottom, right, center, left, numberStrip] = await Promise.all([
      image
        ...
        .extract({
          ...
          top: 0,
          width,
          height: Math.min(topHeight, height),
        })
        .png()
        .toBuffer(),
      image
        ...
        .extract({
          left: 0,
          top: Math.max(0, height - bottomHeight),
          width,
          height: Math.min(bottomHeight, height),
        })
        .png()
        .toBuffer(),
      image
        .clone()
        .extract({
          left: Math.max(0, width - rightWidth),
          top: 0,
          width: Math.min(rightWidth, width),
          height,
        })
        .png()
        .toBuffer(),
  ...
  private async getWorker(): Promise<Tesseract.Worker> {
    if (!this.workerPromise) {
      ...
      const enableLogs =
        (this.configService.get<string>("OCR_DEBUG") ?? "false") === "true";
